...
describe('hasDuplicates', function(){
  it('Describes hasDuplicates as a function', function(){
    expect(typeof(hasDuplicates)).toBe('function');
  });
...
describe('When to return false', function(){
  const arrParameters = [
    {case: hasDuplicates([1, 2, 3, 4]), answer: false},
    {case: hasDuplicates([5, 6, 7, 8]), answer: false},
    {case: hasDuplicates([9, 10, 11, 12]), answer: false}
  ]
  arrParameters.forEach((item, i) => {
    it(`Should return "false" when no repeated matches array number: ${i + 1} return: ${item.answer} `, function(){
      expect(item.case).toEqual(item.answer)
...
describe('When to return true', function(){
  const arrParameters = [
    {case: hasDuplicates([1, 2, 3, 4, 1]), answer: true},
    {case: hasDuplicates([5, 6, 7, 8, 6, 7]), answer: true},
    {case: hasDuplicates([9, 10, 11, 12, 9, 10, 11]), answer: true}
  ]
  arrParameters.forEach((item, i) => {
    it(`Should return "true" when repeated matches are inside the array number: ${i + 1} return: ${item.answer} `, function(){
      expect(item.case).toEqual(item.answer)
...
describe('Returning "false"', function(){
  it('Should return "false" when is injected an empty array as a parameter', function(){
    let arr = [];
